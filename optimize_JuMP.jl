...
Pkg.add("XLSX")
Pkg.add("Missings")
using LinearAlgebra, Random, DataFrames, XLSX, Missings
...
data = data[1:100,:] # keep small number of nodes, for testing
...
m = Model(optimizer_with_attributes(Ipopt.Optimizer, "start_with_resto" => "yes", "linear_solver"=>"mumps","max_iter"=>10))

@NLparameter(m, x0[i=1:N] == 0)
...
JuMP.register(m, :maxfun, 1, maxfun, autodiff=true)
JuMP.register(m, :minfun, 2, minfun, autodiff=true)

@NLexpression(m, myexpr[i=1:N],(1+g0)*(sum(A[j,i]*p[j] for j in 1:N) + c[i] - x0[i]*c[i]) - g0*data_nm.p_bar[i])
@NLconstraint(m,[i=1:N],  minfun(data_nm.p_bar[i], maxfun(myexpr[i])) == p[i] ) 
...
for q=1:2
    for i=1:N
        set_start_value(c[i], csol[i])
        set_start_value(b[i], bsol[i])
        set_start_value(A[i], Asol[i])
    ...
    @testset "check solution" begin
        ...
        @test norm( psol .- min.(data.p_bar, max.((1+g0)*(Asol'*psol .+ csol .- x0vec .* csol) .-g0.*data.p_bar,0.0)) ) < tol
